...
###########################################################################
###########################################################################
### Calculate LRP trends 
lrp_trend_os = calcTrend(lrp_os)
lrp_trend_os_10ye = calcTrend(lrp_os_10ye)

### Calculate ensemble mean of LRP trends
lrp_os_ig_mean = np.nanmean(lrp_trend_os,axis=0)
lrp_os_10ye_ig_mean = np.nanmean(lrp_trend_os_10ye,axis=0)
...
plt.xticks(np.arange(-4,4.02,0.2),map(str,np.round(np.arange(-4,4.02,0.2),2)),fontsize=10)
...
plt.xlabel(r'\textbf{Trend for SPEAR_MED_SSP534OS}',fontsize=9,color='dimgrey')
plt.ylabel(r'\textbf{Trend for ANN Relevance for SPEAR_MED_SSP534OS}',fontsize=9,color='dimgrey')

plt.tight_layout()
plt.savefig(directoryfigure + 'SCATTER_LRP-trendBoth_SPEAR-MED-SSP534OS_v1_%s-%s.png' % (yrmin_os,yrmax_os),dpi=300)
...
plt.xlabel(r'\textbf{Trend for SPEAR_MED_SSP534OS_10ye}',fontsize=9,color='dimgrey')
plt.ylabel(r'\textbf{Trend for ANN Relevance for SPEAR_MED_SSP534OS_10ye}',fontsize=9,color='dimgrey')

plt.tight_layout()
plt.savefig(directoryfigure + 'SCATTER_LRP-trendBoth_SPEAR-MED-SSP534OS_10ye_v1_%s-%s.png' % (yrmin_os_10ye,yrmax_os_10ye),dpi=300)

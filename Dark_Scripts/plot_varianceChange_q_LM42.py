...
def readData(model,reg_name):
    ...
    for e in range(ENS):
        ...
        if any([vari == 'T2M',vari == 'TMAX']):
            ...
            daysall[e,:,:,:,:] = np.concatenate([june,july,august],axis=1) * 1000. # kg/kg to g/kg
            print('Completed: Changed units (kg/kg to g/kg)!')
        else:
...
yearq = np.where((years >= 1981) & (years <= 2010))[0] # 30 years
...
label = r'\textbf{q Var. [(g/kg)$^{2}$]}'
limit = np.arange(0,10.01,0.1)
barlim = np.round(np.arange(0,11,2),2)
...
cs1 = m.pcolormesh(lon2,lat2,var_spearm,vmin=0,vmax=10,latlon=True)
cs1.set_cmap(cmr.jungle_r)
...
cs1 = m.pcolormesh(lon2,lat2,var_lm42m,vmin=0,vmax=10,latlon=True)
cs1.set_cmap(cmr.jungle_r)
...
###############################################################################
label = r'\textbf{q Var. Difference [(g/kg)$^{2}$]}'
...
cs1 = m.pcolormesh(lon2,lat2,var_spearm-var_lm42m,vmin=-4,vmax=4,latlon=True)
...
plt.savefig(directoryfigure + 'varianceChange_q_LM42_1981-2010.png',dpi=300)

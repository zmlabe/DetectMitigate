"""
First version of classification problem for emission scenario_10ye
...
scenarioall = [r'\textbf{HISTORICAL}',r'\textbf{NATURAL}',r'\textbf{SSP5-8.5}',r'\textbf{SSP1-1.9}',r'\textbf{SSP2-4.5}']
shuffletype = 'GAUSS'
###############################################################################
###############################################################################
land_only = False
...
plt.xticks(np.arange(2015,2100+1,10),map(str,np.arange(2015,2100+1,10)),size=8)
plt.yticks(np.arange(0,1.01,0.1),map(str,np.round(np.arange(0,1.01,0.1),2)),size=8)
plt.xlim([2015,2100])   
plt.ylim([0,1.0])           

plt.ylabel(r'\textbf{Confidence}',color='dimgrey',fontsize=8,labelpad=8)
# plt.title(r'\textbf{ANN CONFIDENCE OF SSP534OS_10ye -- %s}' % (variq),color='darkgrey',fontsize=10)

plt.tight_layout()
plt.subplots_adjust(bottom=0.15)
plt.savefig(directoryfigure + 'POSTER_%s_Confidence_10ye.png' % (savename),dpi=300)
...
norm = cccc.BoundaryNorm(np.arange(1,numOfEns+1,1),csm.N)

cs = plt.pcolormesh(allfreq,shading='faceted',edgecolor='w',
                    linewidth=0.01,norm=norm,cmap=csm,clip_on=False)

plt.xticks(np.arange(0.5,87.5,5),np.arange(2015,2101,5),ha='center',va='center',color='dimgrey',size=6)
plt.yticks(np.arange(0.5,5.5,1),scenarioall,ha='right',va='center',color='k',size=12)
yax = ax.get_yaxis()
...
###############################################################################                
cbar_ax1 = fig.add_axes([0.35,0.13,0.3,0.03])                
...
for j, lab in enumerate(range(0,32,1)):
    cbar.ax.text((2 * j+2.9)/2 + 0.09, 0.38, lab,ha='center',va='center',
                 size=4,color='crimson')

plt.tight_layout()
plt.subplots_adjust(bottom=0.3)
plt.savefig(directoryfigure + 'POSTER_%s_NumberOfEnsChoices_10ye.png' % (savename),dpi=300)

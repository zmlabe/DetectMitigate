"""
Function(s) reads in monthly data from MPI_ESM12_HR_LOWS for different variables 
using 10 ensemble members for all time periods using SPEAR_LOW resolution
...
Usage
-----
    [1] read_MPI_ESM12_HR_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)
"""

def read_MPI_ESM12_HR_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper):
    """
    Function reads monthly data from MPI_ESM12_HR_LOWS
    ...
    Usage
    -----
    read_MPI_ESM12_HR_LOWS(directory,vari,sliceperiod,sliceshape,
                             slicenan,numOfEns,timeper)
    """
    print('\n>>>>>>>>>> STARTING read_MPI_ESM12_HR_LOWS function!')
    ...
    for i,ensmember in enumerate(ens):
        filename1 = directory + 'MPI_ESM12_HR_LOWS/monthly/%s/%s_%03d_1850-2100.nc' % (vari,vari,
                                                          ensmember)
        ...
        print('Completed: read *MPI_ESM12_HR_LOWS historical* Ensemble Member --%s-- for %s' % (ensmember,vari))
    ...
    del membersvar
    print('Completed: read all MPI_ESM12_HR_LOWS Members!\n')
    ...
    if slicenan == 'nan':
        ...
        ensshape[np.where(ensshape >= 1e16)] = np.nan 
        ...
        ensshape[np.where(ensshape >= 1e16)] = slicenan
    ...
    if timeper == 'all':
        ...
        print(time[yearhistq])
        histmodel = ensshape[:,yearhistq,:,:]
    elif timeper == 'spear_years':
        yearhistq = np.where((time >= 1921) & (time <= 2100))[0]
        print('SPEAR YEARS')
        print(time[yearhistq])
        histmodel = ensshape[:,yearhistq,:,:]
    elif timeper == 'future':
    ...
    print('>>>>>>>>>> ENDING read_MPI_ESM12_HR_LOWS function!')    
...
# numOfEns = 10
# timeper = 'all'
# lat,lon,var = read_MPI_ESM12_HR_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)

"""
Calculate WPPRECT for 20CRv3

Reference : Warner et al. 2020, GRL
...
for i in range(len(monthsall)):
    ...
    ### Calculate SHI
    lonq = np.where((lonobs >=110) & (lonobs <=140))[0]
    latq = np.where((latobs >=-5) & (latobs <=25))[0]
    anomlon = vardt[:,:,lonq]
    anoms = anomlon[:,latq,:]
    lat2sq = lat2[latq,:]
    lat2s = lat2sq[:,lonq]
    ...
    ### Save index
    directoryoutput = '/work/Zachary.Labe/Data/ClimateIndices/WPPRECT/'
    np.savetxt(directoryoutput + 'WPPRECT-20CRv3_%s_%s_%s-%s_detrended.txt' % (variq,sliceperiod,
                                                                    yearmin,yearmax),
               np.c_[yearsall,shi])
    print('\n========Calculated WPPRECT SST=======\n')

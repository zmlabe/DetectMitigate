"""
Function(s) reads in monthly data from SPEAR_LOW_Hindcast-65lev for different 
...
Usage
-----
    [1] read_SPEAR_LOW_Hindcast65lev(directory,vari,month,slicenan,numOfEns)
"""

def read_SPEAR_LOW_Hindcast65lev(directory,vari,month,slicenan,numOfEns):
    """
    ...
    Usage
    -----
    read_SPEAR_LOW_Hindcast65lev(directory,vari,month,slicenan,numOfEns)
    """
    print('\n>>>>>>>>>> STARTING read_SPEAR_LOW_Hindcast65lev function!')
    ...
    for yr in range(time.shape[0]):
        ...
        directorytotal = directory + 'SPEAR-65lev/raw/%s/Y%s/' % (vari,time[yr])
        ...
        for i,ensmember in enumerate(ens):
            ...
            print('Completed: read *SPEAR_LOW_Hindcast65lev* Ensemble Member --%s-%s-%s--' % (time[yr],monstr,ensmember))
    ...
    print('Completed: read all SPEAR_LOW_Hindcast65lev Members!\n')
    ...
    elif any([vari=='PRECL',vari=='PRECC',vari=='PRECT',vari=='EVAP',vari=='WA',vari=='SNOWRATE']):
    ...
    # output = []
    # for yr in range(ensshape.shape[0]):
    #     outpute = []
    #     for e in range(ensshape.shape[1]):
    #         initial = ensshape[yr,e,:-1,:,:]
    #         lead = ensshape[yr,e,1:,:,:]
    #         diff = lead - initial
    #         outputq = ensshape[yr,e,:-1,:,:] - diff
    #         outpute.append(outputq)
    #     output.append(outpute)       
    # output = np.asarray(output)
    
    # output = np.empty((ensshape.shape))
    # output[:] = np.nan
    # for yr in range(ensshape.shape[0]):
    #     for e in range(ensshape.shape[1]):
    #         output[yr,e,:,:,:] = ensshape[yr,e,:,:,:] - np.nanmean(ensshape[:,:,:,:,:],axis=(0,1))
    output = ensshape - np.nanmean(ensshape[:,:,:,:,:],axis=(0,1))
            
    print('Shape of output FINAL = ', output.shape,[[output.ndim]])
    print('>>>>>>>>>> ENDING read_SPEAR_LOW_Hindcast65lev function!')    
...
# import calc_Utilities as UT
# import cmocean
# directory = '/work/Zachary.Labe/Data/SPEAR/Hindcasts/SPEAR_LOW/SPEAR-65lev/'
# vari = 'T2M'
# month = 'Mar'
# slicenan = 'nan'
# numOfEns = 15
# lat,lon,var= read_SPEAR_LOW_Hindcast65lev(directory,vari,month,slicenan,numOfEns)
...
# import read_ERA5_monthly1x1_hindcastSPEAR_65lev as ER
# late,lone,leve,vare = ER.read_ERA5_monthlyHindcast(vari,'/work/Zachary.Labe/Data/',month,slicenan,'surface')

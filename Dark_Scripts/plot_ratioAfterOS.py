...
ave_os_10ye = UT.calc_weightedAve(spear_osm_10ye,lat2)
...
ave_os1 = np.nanmean(ave_os[os_yr-5:os_yr+5])
ave_os1_10ye = np.nanmean(ave_os_10ye[os_10ye_yr-5:os_10ye_yr+5])
...
ave_os2 = np.nanmean(ave_os[end+os_yr-5:end+os_yr+5])
ave_os2_10ye = np.nanmean(ave_os_10ye[end+os_10ye_yr-5:end+os_10ye_yr+5])

### Calculate changes in epochs
change_os = spear_os2 - spear_os1
change_os_10ye = spear_os2_10ye - spear_os1_10ye

avechange_os = ave_os2 - ave_os1
avechange_os_10ye = ave_os2_10ye - ave_os1_10ye
...
cs = m.contourf(lon2,lat2,os_ratio,limit,
...
### Make the plot continuous
cs = m.contourf(lon2,lat2,os_10ye_ratio,limit,
                extend='both',latlon=True)
...
if variq == 'PRECT':
    label = r'\textbf{PRECIPITATION CHANGE RELATIVE TO GLOBAL AVERAGE [mm/day]}' 
elif variq == 'T2M':
    label = r'\textbf{TEMPERATURE CHANGE RELATIVE TO GLOBAL AVERAGE [$^{\circ}$C]}' 
...
### Save figure 
plt.tight_layout()   
plt.savefig(directoryfigure + 'ratioAfterOS_%s_in_%syrs.png' % (variq,end),dpi=300)

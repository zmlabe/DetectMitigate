"""
Function(s) reads in monthly data from MPI_ESM12_LE for different variables 
using 30 ensemble members for all time periods using SPEAR_LOW resolution
...
Usage
-----
    [1] read_MPI_ESM12_LE(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)
"""

def read_MPI_ESM12_LE(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper):
    """
    Function reads monthly data from MPI_ESM12_LE
    ...
    Usage
    -----
    read_MPI_ESM12_LE(directory,vari,sliceperiod,sliceshape,
                             slicenan,numOfEns,timeper)
    """
    print('\n>>>>>>>>>> STARTING read_MPI_ESM12_LE function!')
    ...
    for i,ensmember in enumerate(ens):
        filename1 = directory + 'MPI_ESM12_LE/monthly/%s/%s_%03d_1850-2100.nc' % (vari,vari,
                                                          ensmember)
        ...
        print('Completed: read *MPI_ESM12_LE historical* Ensemble Member --%s-- for %s' % (ensmember,vari))
    ...
    del membersvar
    print('Completed: read all MPI_ESM12_LE Members!\n')
    ...
    print('>>>>>>>>>> ENDING read_MPI_ESM12_LE function!')    
...
# numOfEns = 30
# timeper = 'all'
# lat,lon,var = read_MPI_ESM12_LE(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)

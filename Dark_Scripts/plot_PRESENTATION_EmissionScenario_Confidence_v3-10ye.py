...
scenarioall = [r'\textbf{HISTORICAL}',r'\textbf{NATURAL}',r'\textbf{SSP585}',r'\textbf{SSP119}',r'\textbf{SSP245}']
shuffletype = 'GAUSS'
###############################################################################
###############################################################################
land_only = False
...
plt.savefig(directoryfigure + 'PRESENTATION_%s_Confidence_10ye.png' % (savename),dpi=300)
...
cs = plt.pcolormesh(allfreq,shading='faceted',edgecolor='k',
                    linewidth=0.01,norm=norm,cmap=csm,clip_on=False)

plt.xticks(np.arange(0.5,87.5,5),np.arange(2015,2101,5),ha='center',va='center',color='darkgrey',size=6)
plt.yticks(np.arange(0.5,5.5,1),scenarioall,ha='right',va='center',color='w',size=12)
yax = ax.get_yaxis()
...
cbar.set_label(r'\textbf{NUMBER OF ENSEMBLE MEMBERS}',color='w',labelpad=7,fontsize=18)
...
plt.savefig(directoryfigure + 'PRESENTATION_%s_NumberOfEnsChoices_10ye.png' % (savename),dpi=300)

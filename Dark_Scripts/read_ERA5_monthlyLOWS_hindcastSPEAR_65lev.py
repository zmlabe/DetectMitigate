"""
Function reads in monthly data from ERA5 for SPEAR_LOW to better evaluate
...
Usage
-----
    [1] read_ERA5LOWS_monthlyHindcast(variq,directory,month,slicenan,level)
"""

def read_ERA5LOWS_monthlyHindcast(variq,directory,month,slicenan,level):
    """
    Function reads monthly data from ERA5-LOWS
    ...
    Usage
    -----
    lat,lon,lev,var = read_ERA5LOWS_monthlyHindcast(variq,directory,month,slicenan,level)
    """
    print('\n>>>>>>>>>> STARTING read_ERA5LOWS_monthlyHindcast (LOWS) function!')
    ...
    print('>>>>>>>>>> ENDING read_ERA5LOWS_monthlyHindcast (LOWS) function for -- %s!' % variq)
...
# slicenan = 'nan'
# level = 'surface'
# lat,lon,lev,var1 = read_ERA5LOWS_monthlyHindcast(variq,directory,month,slicenan,level)
# lon2,lat2 = np.meshgrid(lon,lat)
# ave1 = UT.calc_weightedAve(var1,lat2)

# lat,lon,lev,var2 = read_ERA5LOWS_monthlyHindcast(variq,directory,'Feb',slicenan,level)
# lon2,lat2 = np.meshgrid(lon,lat)
# ave2 = UT.calc_weightedAve(var2,lat2)

# lat,lon,lev,var3 = read_ERA5LOWS_monthlyHindcast(variq,directory,'Mar',slicenan,level)
# lon2,lat2 = np.meshgrid(lon,lat)

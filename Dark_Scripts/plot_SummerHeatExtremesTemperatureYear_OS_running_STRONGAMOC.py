...
for the count of summer temperature extremes but with running mean for the AMOC
runs
...
experimentnames = ['SSP5-34OS','SSP5-34OS_AMOC','SSP5-34OS minus SSP5-34OS_AMOC']
seasons = ['JJA']
slicemonthnamen = ['JJA']
...
spear_osm_AMOC_meann = np.append(spear_osm_meann[:(len(years)-len(spear_osm_AMOC_meanq)),:,:],spear_osm_AMOC_meanq,axis=0)

### Add on years 1921-2014
spear_osm_mean = np.append(spearslice,spear_osm_meann,axis=0)
spear_osm_AMOC_mean = np.append(spearslice,spear_osm_AMOC_meann,axis=0)
...
for i in range(lats.shape[0]):
    for j in range(lons.shape[0]):
        ...
        gridpoint_osr_AMOC = spear_osm_AMOC_mean[:,i,j]
        
        ### Calculate running mean
        gridpoint_os = pd.Series(gridpoint_osr).rolling(window=rolling_years).mean().values
        gridpoint_os_AMOC = pd.Series(gridpoint_osr_AMOC).rolling(window=rolling_years).mean().values
        
        gridpoint_os = gridpoint_os[-len(years):]
        gridpoint_os_AMOC = gridpoint_os_AMOC[-len(years):]
        
        maxindex_os = np.nanargmax(gridpoint_os)
        maxindex_os_AMOC = np.nanargmax(gridpoint_os_AMOC)
...
for i in range(lats.shape[0]):
    for j in range(lons.shape[0]):
        ...
        if (maxindex_os_AMOC >= wherebegin_os_AMOC) and (maxindex_os >= wherebegin_os):
            ...
            IndexmaxAfterOS_AMOC = np.nanargmax(gridpoint_os_AMOC[:])
...
spear_osm_AMOC_meanS = spear_osm_AMOC_mean[:,latqS,:]
spear_osm_meanN = spear_osm_mean[:,latqN,:]
spear_osm_AMOC_meanN = spear_osm_AMOC_mean[:,latqN,:]

os_South = UT.calc_weightedAve(spear_osm_meanS,lat2S)
os_North = UT.calc_weightedAve(spear_osm_meanN,lat2N)

os_AMOC_South = UT.calc_weightedAve(spear_osm_AMOC_meanS,lat2S)
os_AMOC_North = UT.calc_weightedAve(spear_osm_AMOC_meanN,lat2N)

### Check hemisphere plotsd
plt.figure()
plt.plot(os_South)
plt.plot(os_AMOC_South)

plt.figure()
plt.plot(os_North)
plt.plot(os_AMOC_North)
...
plt.savefig(directoryfigure + 'SummerHeatExtremesTemperatureYear_OS-T2M_%s_%s_running-%syr_%s_AMOC_p1Sv.png' % (variq,seasons[0],rolling_years,varcount),dpi=300)
...
plt.savefig(directoryfigure + 'SummerHeatExtremesTemperatureYear-Difference_OS-T2M_%s_%s_running-%syr_%s_AMOC_p1Sv.png' % (variq,seasons[0],rolling_years,varcount),dpi=300)
...
ax1.annotate(r'\textbf{argmax(SSP5-34OS_AMOC) - argmin(SSP5-34OS)}',xy=(0,0),xytext=(0.5,1.10),
              textcoords='axes fraction',color='dimgrey',fontsize=14,
              rotation=0,ha='center',va='center')
...
plt.savefig(directoryfigure + 'SummerHeatExtremesTemperatureYear-Benefit_OS-T2M_%s_%s_running-%syr_%s_AMOC_p1Sv.png' % (variq,seasons[0],rolling_years,varcount),dpi=300)
...
plt.savefig(directoryfigure + 'SummerHeatExtremesTemperatureYear-BenefitMask_OS-%s_%s_running-%syr_%s_AMOC_p1Sv.png' % (variq,seasons[0],rolling_years,varcount),dpi=300)
...
plt.savefig(directoryfigure + 'SummerHeatExtremesTemperatureYear-BenefitMask_OS-%s_%s_running-%syr_%s_land_AMOC_p1Sv.png' % (variq,seasons[0],rolling_years,varcount),dpi=300)

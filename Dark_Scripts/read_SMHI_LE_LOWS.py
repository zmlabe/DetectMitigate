"""
Function(s) reads in monthly data from SMHI_LE_LOWS for different variables 
...
Usage
-----
    [1] read_SMHI_LE_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)
"""

def read_SMHI_LE_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper):
    """
    Function reads monthly data from SMHI_LE_LOWS
    ...
    Usage
    -----
    read_SMHI_LE_LOWS(directory,vari,sliceperiod,sliceshape,
                             slicenan,numOfEns,timeper)
    """
    print('\n>>>>>>>>>> STARTING read_SMHI_LE_LOWS function!')
    ...
    for i,ensmember in enumerate(ens):
        filename1 = directory + 'SMHI_LE_LOWS/monthly/%s/%s_%03d_1970-2100.nc' % (vari,vari,ensmember)
        ...
        print('Completed: read *SMHI_LE_LOWS historical* Ensemble Member --%s-- for %s' % (ensmember,vari))
    ...
    del membersvar
    print('Completed: read all SMHI_LE_LOWS Members!\n')
    ...
    ###########################################################################
    ### Select years of analysis (1970-2100)
    if timeper == 'all':
    ...
    print('>>>>>>>>>> ENDING read_SMHI_LE_LOWS function!')    
...
# numOfEns = 50
# timeper = 'all'
# lat,lon,var = read_SMHI_LE_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)

"""
Calculate heat extremes regressed on the AMO index
...
x,y = m(lonnew2,latnew2)
cs1 = m.contourf(x,y,spear_am_na[0,0,:,:],300,extend='both',cmap=cmocean.cm.thermal,zorder=11)
...
### Calculate index - filter
ave_spear_filter = savgol_filter(ave_spear,10,3,axis=1)
ave_spear_os_filter = savgol_filter(ave_spear_os,10,3,axis=1)
ave_spear_os10ye_filter = savgol_filter(ave_spear_os10ye,10,3,axis=1)

### Save output
directorydata = '/work/Zachary.Labe/Data/ClimateIndices/AMO/'
if detrend == True:
    np.savez(directorydata + 'AMO_SST_SPEAR-MED_%s_2015-2100_detrend.npz' % monthlychoice,
                  spear_unfilter=ave_spear,spear_filter=ave_spear_filter,lat=latnew,lon=lonnew,years=years)
    np.savez(directorydata + 'AMO_SST_SPEAR-MED-SSP534OS_%s_2015-2100_detrend.npz' % monthlychoice,
                  spear_os_unfilter=ave_spear_os,spear_os_filter=ave_spear_os_filter,lat=latnew,lon=lonnew,years=years)
    np.savez(directorydata + 'AMO_SST_SPEAR-MED-SSP534OS-10ye_%s_2015-2100_detrend.npz' % monthlychoice,
                  spear_os10ye_unfilter=ave_spear_os10ye,spear_os10ye_filter=ave_spear_os10ye_filter,lat=latnew,lon=lonnew,years=years)
elif detrend == False:
    np.savez(directorydata + 'AMO_SST_SPEAR-MED_%s_2015-2100.npz' % monthlychoice,
                  spear_unfilter=ave_spear,spear_filter=ave_spear_filter,lat=latnew,lon=lonnew,years=years)
    np.savez(directorydata + 'AMO_SST_SPEAR-MED-SSP534OS_%s_2015-2100.npz' % monthlychoice,
                  spear_os_unfilter=ave_spear_os,spear_os_filter=ave_spear_os_filter,lat=latnew,lon=lonnew,years=years)
    np.savez(directorydata + 'AMO_SST_SPEAR-MED-SSP534OS-10ye_%s_2015-2100.npz' % monthlychoice,
                  spear_os10ye_unfilter=ave_spear_os10ye,spear_os10ye_filter=ave_spear_os10ye_filter,lat=latnew,lon=lonnew,years=years)
...
###############################################################################
###############################################################################
### Regress heatwave day anomalies onto AMO index
...
for ee in range(len(spear_amho)):
    slope,intercept,rvalue,pvalue,stderr = regressData(ave_spear_filter[ee,:],spear_amho[ee,:,:,:],'SPEAR_MED')
    ensRegr[ee,:,:] = slope
    ensPVal[ee,:,:] = pvalue
    ensRVal[ee,:,:] = rvalue
...
for ee in range(len(spear_aosmho)):
    slope_os,intercept_os,rvalue_os,pvalue_os,stderr_os = regressData(ave_spear_os_filter[ee,:],spear_aosmho[ee,:,:,:],'SPEAR_MED_SSP534OS')
...
for ee in range(len(spear_aosm_10yeho)):
    slope_os10ye,intercept_os10ye,rvalue_os10ye,pvalue_os10ye,stderr_os10ye = regressData(ave_spear_os10ye_filter[ee,:],spear_aosm_10yeho[ee,:,:,:],'SPEAR_MED_SSP534OS_10ye')
...
label = r'\textbf{Regression Coefficients [AMO; days]}'
...
plt.savefig(directoryfigure + 'Regression_AMO-HeatDays_TMAX_%s_2015-2100.png' % (monthlychoice),dpi=300)

"""
Calculate GMST for the different OS2040 simulations
...
experimentnames = ['SSP5-34OS','SSP370_OS2040a','DIFFERENCE [a-b]']
...
spear_ssp370,lats,lons = read_primary_dataset(variq,'SPEAR_MED_Scenario',monthlychoice,'SSP370',lat_bounds,lon_bounds)
...
spear_osm_OS2040a,lats,lons = read_primary_dataset(variq,'SPEAR_MED_SSP370_OS2040a',monthlychoice,'SSP534OS_10ye',lat_bounds,lon_bounds)
lon2,lat2 = np.meshgrid(lons,lats)

### Calculate anomalies
yearq = np.where((years >= 2015) & (years <= 2029))[0]
climo_spear = np.nanmean(spear_m[:,yearq,:,:],axis=1)
climo_spear_ssp370 = np.nanmean(spear_ssp370[:,yearq,:,:],axis=1)
climo_osspear = np.nanmean(spear_osm[:,yearq,:,:],axis=1)
climo_os10yespear = np.nanmean(spear_osm_10ye[:,yearq,:,:],axis=1)
climo_osOS2040aspear = np.nanmean(spear_osm_OS2040a[:,yearq,:,:],axis=1)

spear_am = spear_m - climo_spear[:,np.newaxis,:,:]
spear_am_ssp370 = spear_ssp370 - climo_spear_ssp370[:,np.newaxis,:,:]
spear_aosm = spear_osm - climo_osspear[:,np.newaxis,:,:]
spear_aosm_10ye = spear_osm_10ye - climo_os10yespear[:,np.newaxis,:,:]
spear_aosm_OS2040a = spear_osm_OS2040a - climo_osOS2040aspear[:,np.newaxis,:,:]

### Calculate global means
ave_GLOBE = UT.calc_weightedAve(spear_am,lat2)
ave_ssp370_GLOBE  = UT.calc_weightedAve(spear_am_ssp370 ,lat2)
ave_os_GLOBE = UT.calc_weightedAve(spear_aosm,lat2)
ave_os_10ye_GLOBE = UT.calc_weightedAve(spear_aosm_10ye,lat2)
ave_os_OS2040a_GLOBE = UT.calc_weightedAve(spear_aosm_OS2040a,lat2)
...
ave_GLOBE_ssp370_avg = np.nanmean(ave_ssp370_GLOBE,axis=0)
ave_GLOBE_ssp370_min = np.nanmin(ave_ssp370_GLOBE,axis=0)
ave_GLOBE_ssp370_max = np.nanmax(ave_ssp370_GLOBE,axis=0)
...
ave_os_OS2040a_GLOBE_avg = np.nanmean(ave_os_OS2040a_GLOBE,axis=0)
ave_os_OS2040a_GLOBE_min = np.nanmin(ave_os_OS2040a_GLOBE,axis=0)
ave_os_OS2040a_GLOBE_max = np.nanmax(ave_os_OS2040a_GLOBE,axis=0)
...
plt.plot(years,ave_GLOBE_ssp370_avg,linestyle='-',linewidth=1,color='salmon',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP370}')   
...
plt.plot(years,ave_os_OS2040a_GLOBE_avg,linestyle='-',dashes=(1,0.3),linewidth=1,color='darkorange',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP370_OS2040a}') 
...
plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_Globe_T2M_OS2040a.png',dpi=300)
...
ave_ssp370_GLOBE_avgh = np.nanmean(ave_GLOBEh_ssp370,axis=0)
ave_ssp370_GLOBE_minh = np.nanmin(ave_GLOBEh_ssp370,axis=0)
ave_ssp370_GLOBE_maxh = np.nanmax(ave_GLOBEh_ssp370,axis=0)
...
ave_os_OS2040a_GLOBE_avgh = np.nanmean(ave_os_OS2040a_GLOBEh,axis=0)
ave_os_OS2040a_GLOBE_minh = np.nanmin(ave_os_OS2040a_GLOBEh,axis=0)
ave_os_OS2040a_GLOBE_maxh = np.nanmax(ave_os_OS2040a_GLOBEh,axis=0)
...
plt.plot(years,ave_GLOBE_avgh,linestyle='-',linewidth=2,color='maroon',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP585}')   
plt.plot(years,ave_ssp370_GLOBE_avgh,linestyle='-',linewidth=1,color='salmon',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP370}')  

plt.plot(years,ave_os_GLOBE_avgh,linestyle='-',linewidth=2,color='darkslategrey',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP534OS}') 
plt.plot(years,ave_os_OS2040a_GLOBE_avgh,linestyle='-',dashes=(1,0.3),linewidth=1,color='darkorange',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP370_OS2040a}')     
...
plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_Globe_T2M_historicalbaseline_OS2040a.png',dpi=300)

###############################################################################
###############################################################################
###############################################################################
### Plot historical baseline - zoom
fig = plt.figure()
ax = plt.subplot(111)
...
plt.fill_between(years,ave_os_GLOBE_minh,ave_os_GLOBE_maxh,facecolor='deepskyblue',zorder=1,
                 alpha=0.4,edgecolor='none',clip_on=False)
plt.fill_between(years,ave_os_OS2040a_GLOBE_minh,ave_os_OS2040a_GLOBE_maxh,facecolor='crimson',zorder=1,
                 alpha=0.4,edgecolor='none',clip_on=False)
plt.plot(years,ave_os_GLOBE_avgh,linestyle='-',linewidth=2,color='deepskyblue',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP534OS}') 
plt.plot(years,ave_os_OS2040a_GLOBE_avgh,linestyle='--',dashes=(1,0.3),linewidth=2,color='crimson',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP370_OS2040a}')     
...
plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_Globe_T2M_historicalbaseline_OS2040a_zoom.png',dpi=300)

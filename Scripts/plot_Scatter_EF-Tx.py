"""
Examine relationship between evaporative fraction and heatwaves
...
spear_mALL,lats,lons = read_primary_dataset(variq[0],'SPEAR_MED',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_hALL,lats,lons = read_primary_dataset(variq[0],'SPEAR_MED_ALLofHistorical',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_osmALL,lats,lons = read_primary_dataset(variq[0],'SPEAR_MED_Scenario',monthlychoice,'SSP534OS',lat_bounds,lon_bounds)
spear_osm_10yeALL,lats,lons = read_primary_dataset(variq[0],'SPEAR_MED_SSP534OS_10ye',monthlychoice,'SSP534OS_10ye',lat_bounds,lon_bounds)

### Mask over the USA
spear_amS,maskobs = dSS.mask_CONUS(spear_mALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_ahS,maskobs = dSS.mask_CONUS(spear_hALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_aosmS,maskobs = dSS.mask_CONUS(spear_osmALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_aosm_10yeS,maskobs = dSS.mask_CONUS(spear_osm_10yeALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)

spear_mALLla,lats,lons = read_primary_dataset(variq[1],'SPEAR_MED',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_hALLla,lats,lons = read_primary_dataset(variq[1],'SPEAR_MED_ALLofHistorical',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_osmALLla,lats,lons = read_primary_dataset(variq[1],'SPEAR_MED_Scenario',monthlychoice,'SSP534OS',lat_bounds,lon_bounds)
spear_osm_10yeALLla,lats,lons = read_primary_dataset(variq[1],'SPEAR_MED_SSP534OS_10ye',monthlychoice,'SSP534OS_10ye',lat_bounds,lon_bounds)
spear_amla,maskobs = dSS.mask_CONUS(spear_mALLla,np.full((spear_mALLla.shape[1],spear_mALLla.shape[2],spear_mALLla.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_ahla,maskobs = dSS.mask_CONUS(spear_hALLla,np.full((spear_mALLla.shape[1],spear_mALLla.shape[2],spear_mALLla.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_aosmla,maskobs = dSS.mask_CONUS(spear_osmALLla,np.full((spear_mALLla.shape[1],spear_mALLla.shape[2],spear_mALLla.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_aosm_10yela,maskobs = dSS.mask_CONUS(spear_osm_10yeALLla,np.full((spear_mALLla.shape[1],spear_mALLla.shape[2],spear_mALLla.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)

### Calculate latent heat (H = p*lv*E)
spear_mALLlal = 1000 * 2.45e6 * spear_mALLla * (1/86400) * (1/1000)
spear_hALLlal = 1000 * 2.45e6 * spear_hALLla * (1/86400) * (1/1000)
spear_osmALLlal = 1000 * 2.45e6 * spear_osmALLla * (1/86400) * (1/1000)
spear_osm_10yeALLlal = 1000 * 2.45e6 * spear_osm_10yeALLla * (1/86400) * (1/1000)

### Calculate evaporative fraction
spear_am = spear_mALLlal/(spear_amS + spear_mALLlal)
spear_ah = spear_hALLlal/(spear_ahS + spear_hALLlal)
spear_aosm = spear_osmALLlal/(spear_aosmS + spear_osmALLlal)
spear_aosm_10ye = spear_osm_10yeALLlal/(spear_aosm_10yeS + spear_osm_10yeALLlal)
...
### Calculate anomalies for global temperature peaks
yearq = np.where((yearsh >= 1921) & (yearsh <= 1950))[0]
climoh_spear = np.nanmean(np.nanmean(spear_ah[:,yearq,:,:],axis=1),axis=0)
...
leg = plt.legend(shadow=False,fontsize=7,loc='upper right',
      bbox_to_anchor=(1,1.02),fancybox=True,ncol=1,frameon=False,
...
plt.yticks(np.arange(0,1.1,0.1),map(str,np.round(np.arange(0,1.1,0.1),2)),fontsize=10)
plt.xlim([0,60])
plt.ylim([0.5,0.8])

plt.xlabel(r'\textbf{Count of Tn90 days in JJA}',fontsize=11,color='dimgrey')
plt.ylabel(r'\textbf{Evaporative Fraction [unitless]}',fontsize=11,color='dimgrey')
...
plt.tight_layout()
plt.savefig(directoryfigure + 'Scatter_EF-Tn90_OS.png',dpi=300)

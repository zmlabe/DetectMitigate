...
ax1.annotate(r'\textbf{ADDED BENEFIT BY MITIGATING 10 YEARS EARLIER}',xy=(0,0),xytext=(0.5,1.08),
              textcoords='axes fraction',color='k',fontsize=16,
              rotation=0,ha='center',va='center')
    
###############################################################################
cbar_ax1 = fig.add_axes([0.36,0.12,0.3,0.03])                
cbar1 = fig.colorbar(cs1,cax=cbar_ax1,orientation='horizontal',
                    extend='max',extendfrac=0.07,drawedges=False)
cbar1.set_label(label,fontsize=9,color='k',labelpad=1.4)  
cbar1.set_ticks(barlim)
cbar1.set_ticklabels(list(map(str,barlim)))
cbar1.ax.tick_params(axis='x', size=.01,labelsize=5,color='k')
cbar1.outline.set_edgecolor('dimgrey')

plt.tight_layout()

plt.savefig(directoryfigure + 'POSTER_SummerHeatExtremesTemperatureYear-BenefitMask_OS-%s_%s_%s_land_%sFilterSavGol.png' % (variq,seasons[0],varcount,Nfilter),dpi=600,
            transparent=True)

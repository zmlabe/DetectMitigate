"""
Function(s) reads in monthly data from SPEAR_MED_NOAER for different variables 
...
Usage
-----
    [1] read_SPEAR_MED_NOAER(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)
"""

def read_SPEAR_MED_NOAER(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper):
    """
    Function reads monthly data from SPEAR_MED_NOAER
    ...
    Usage
    -----
    read_SPEAR_MED_NOAER(directory,vari,sliceperiod,sliceshape,
                             slicenan,numOfEns,timeper)
    """
    print('\n>>>>>>>>>> STARTING read_SPEAR_MED_NOAER function!')
    ...
    for i,ensmember in enumerate(ens):
        filename = directory + '%s/%s_%02d_1921-2020.nc' % (vari,vari,
                                                          ensmember)
        ...
        print('Completed: read *SPEAR_MED_NOAER historical* Ensemble Member --%s--' % ensmember)
    ...
    print('Completed: read all SPEAR_MED_NOAER Members!\n')
    ...
    ###########################################################################
    ### Select years of analysis (1921-2020)
    if timeper == 'all':
        ...
        yearhistq = np.where((time >= 1921) & (time <= 2020))[0]
        print('HISTORICAL YEARS')
        print(time[yearhistq])
        histmodel = ensshape[:,yearhistq,:,:]
    else:
    ...
    print('>>>>>>>>>> ENDING read_SPEAR_MED_NOAER function!')    
...
# import calc_Utilities as UT
# directory = '/work/Zachary.Labe/Data/SPEAR/SPEAR_MED_NOAER/monthly/'
# vari = 'TMIN'
# sliceperiod = 'annual'
# sliceshape = 4
# slicenan = 'nan'
# numOfEns = 12
# timeper = 'all'
# lat,lon,var = read_SPEAR_MED_NOAER(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)

"""
Function reads in monthly data from NClimGrid for LOWS data
...
Usage
-----
    [1] read_NClimGrid_monthlyLOWS(variq,directory,sliceperiod,sliceyear,
                                   sliceshape,slicenan)
"""

def read_NClimGrid_monthlyLOWS(variq,directory,sliceperiod,sliceyear,sliceshape,slicenan):
    """
    Function reads monthly data from NClimGrid on SPEAR_LOW grid
    ...
    Usage
    -----
    lat,lon,lev,var = read_NClimGrid_monthlyLOWS(variq,directory,sliceperiod,sliceyear,
                                                 sliceshape,slicenan)
    """
    print('\n>>>>>>>>>> STARTING read_NClimGrid_monthlyLOWS function!')
    ...
    filename = 'NClimGrid_LOWS/%s_1895-2021.nc' % variq
    ...
    ###########################################################################
    ### Change years
    yearhistq = np.where((time >= 1895) & (time <= 2021))[0]
    print(time[yearhistq])
    histmodel = varshape[yearhistq,:,:]
        
    print('>>>>>>>>>> ENDING read_NClimGrid_monthlyLOWS function for -- %s!' % variq)
...
# sliceshape = 3
# slicenan = 'nan'
# lat,lon,var = read_NClimGrid_monthlyLOWS(variq,directory,sliceperiod,

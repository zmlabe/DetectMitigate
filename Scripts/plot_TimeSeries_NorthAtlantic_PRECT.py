...
###############################################################################
###############################################################################
### Calculate mean PRECT over the North Atlantic
latmin = 45
latmax = 60
lonmin = 310
lonmax = 340
...
plt.yticks(np.round(np.arange(0,11,0.1),2),np.round(np.arange(0,11,0.1),2))
plt.xlim([2015,2100])
plt.ylim([3,4.4])

plt.ylabel(r'\textbf{North Atlantic PRECT [mm/day}',
            fontsize=10,color='k')
plt.title(r'\textbf{NORTH ATLANTIC %s' % variq,fontsize=15,color='dimgrey')

plt.tight_layout()
plt.savefig(directoryfigure + 'NorthAtlantic_%s_EmissionScenarios_%s.png' % (variq,monthlychoice),dpi=300)

###############################################################################
###############################################################################
###############################################################################
### Calculate mean PRECT over the larger North Atlantic
latmin = 55
latmax = 72
lonmin = 320
lonmax = 359.99
...
plt.ylabel(r'\textbf{NORTHERN North Atlantic PRECT [mm/day]}',
           fontsize=10,color='k')
plt.title(r'\textbf{NORTHERN NORTH ATLANTIC %s' % variq,fontsize=15,color='dimgrey')

plt.tight_layout()
plt.savefig(directoryfigure + 'NorthNorthAtlantic_%s_EmissionScenarios_%s.png' % (variq,monthlychoice),dpi=300)

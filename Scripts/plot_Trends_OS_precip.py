"""
Calculate trend for precipitation in OS runs
...
label = r'\textbf{Trend in precipitation [mm/day/decade] - 30 years after peak warming}' 
...
plt.savefig(directoryfigure + 'Trends_OS_Precipitation_%s_CONUS.png' % (monthlychoice),dpi=300)
...
barlim = np.round(np.arange(-0.2,0.21,0.1),2)
limit = np.arange(-0.2,0.201,0.001)
label = r'\textbf{Trend in precipitation [mm/day/decade] - 2071-2100}' 
...
plt.savefig(directoryfigure + 'Trends_OS_Precipitation_%s_CONUS_last30yrs.png' % (monthlychoice),dpi=300)

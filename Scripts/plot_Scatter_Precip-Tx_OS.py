"""
Examine relationship between precipitation and heatwaves
...
# nsize = 15
# before_os = np.nanmean(count90_osHEAT[:,os_yr-nsize:os_yr,:,:],axis=(0))
# before_os10ye = np.nanmean(count90_os10yeHEAT[:,os_10ye_yr-nsize:os_10ye_yr,:,:],axis=(0))

# before_os_vari = np.nanmean(spear_aosm[:,os_yr-nsize:os_yr,:,:],axis=(0))
# before_os_10ye_vari = np.nanmean(spear_aosm_10ye[:,os_yr-nsize:os_yr,:,:],axis=(0))

# beforeMean_os = UT.calc_weightedAve(before_os,latus2)
# beforeMean_os_10ye = UT.calc_weightedAve(before_os,latus2)
# beforeMean_os_vari = UT.calc_weightedAve(before_os_vari,latus2)
# beforeMean_os_10ye_vari = UT.calc_weightedAve(before_os_vari,latus2)

# ### Calculate epochs - Peak
# afterPeak_os = np.nanmean(count90_osHEAT[:,yrmax_os:yrmax_os+nsize,:,:],axis=(0))
# afterPeak_os10ye = np.nanmean(count90_os10yeHEAT[:,yrmax_os_10ye:yrmax_os_10ye+nsize,:,:],axis=(0))

# after_os_vari = np.nanmean(spear_aosm[:,yrmax_os:yrmax_os+nsize,:,:],axis=(0))
# after_os_10ye_vari = np.nanmean(spear_aosm_10ye[:,yrmax_os_10ye:yrmax_os_10ye+nsize,:,:],axis=(0))

# afterMean_os = UT.calc_weightedAve(afterPeak_os,latus2)
# afterMean_os_10ye = UT.calc_weightedAve(afterPeak_os,latus2)
# afterMean_os_vari = UT.calc_weightedAve(after_os_vari,latus2)
# afterMean_os_10ye_vari = UT.calc_weightedAve(after_os_vari,latus2)

# ### Calculate epochs - Last
# end_os = np.nanmean(count90_osHEAT[:,-nsize:,:,:],axis=(0))
# end_os10ye = np.nanmean(count90_os10yeHEAT[:,-nsize:,:,:],axis=(0))

# end_os_vari = np.nanmean(spear_aosm[:,-nsize:,:,:],axis=(0))
# end_os_10ye_vari = np.nanmean(spear_aosm_10ye[:,-nsize:,:,:],axis=(0))

# endMean_os = UT.calc_weightedAve(end_os,latus2)
# endMean_os_10ye = UT.calc_weightedAve(end_os,latus2)
# endMean_os_vari = UT.calc_weightedAve(end_os_vari,latus2)
# endMean_os_10ye_vari = UT.calc_weightedAve(end_os_vari,latus2)
...
plt.yticks(np.arange(-3,3.1,0.2),map(str,np.round(np.arange(-3,3.1,0.2),2)),fontsize=10)
plt.xlim([0,60])
plt.ylim([-1,1.2])

plt.xlabel(r'\textbf{Count of Tn90 days in JJA}',fontsize=11,color='dimgrey')
plt.ylabel(r'\textbf{Precipitation Anomaly [mm/day]}',fontsize=11,color='dimgrey')
...
plt.tight_layout()
plt.savefig(directoryfigure + 'Scatter_Precip-Tn90_OS.png',dpi=300)

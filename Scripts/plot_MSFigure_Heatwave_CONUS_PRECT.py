...
spear_SSP245mALL,lats,lons = read_primary_dataset(variq,'SPEAR_MED_Scenario',monthlychoice,'SSP245',lat_bounds,lon_bounds)
...
plt.yticks(np.round(np.arange(-100,101,0.5),2),np.round(np.arange(-100,101,0.5),2))
plt.xlim([1920,2100])
plt.ylim([-1,1])

plt.ylabel(r'\textbf{Total Precipitation Anomaly [mm/day]}',fontsize=7,color='k')

plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_CONUS_PRECT_historicalbaseline_%s_%s.png' % (seasons[0],reg_name),dpi=300)

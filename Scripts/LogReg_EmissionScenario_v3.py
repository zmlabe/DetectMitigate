"""
Third version of classification problem for emission scenario, but
...
###############################################################################
###############################################################################
def loadmodel(Xtrain,Xval,Ytrain,Yval,random_network_seed,n_epochs,batch_size,lr_here,ridgePenalty,actFun,class_weight,input_shape,output_shape):
    print('----LR Training: learning rate = '+str(lr_here)+'; activation = '+actFun+'; batch = '+str(batch_size) + '----')
...
model,history = loadmodel(XtrainS,XvalS,Ytrain,Yval,random_network_seed,n_epochs,batch_size,lr_here,ridgePenalty,actFun,class_weight,input_shape,output_shape)
...
###############################################################################
###############################################################################
### Start saving everything, including the LR
dirname = '/work/Zachary.Labe/Research/DetectMitigate/savedModels'
savename = 'LRv3_EmissionScenario_' + variq + '_' + reg_name + '_' + monthlychoice + '_' + actFun + '_L2_'+ str(ridgePenalty)+ '_LR_' + str(lr_here)+ '_Batch'+ str(batch_size)+ '_Iters' + str(n_epochs) + '_' + 'SegSeed' + str(random_segment_seed) + '_NetSeed'+ str(random_network_seed) 
...
lrpobsz = LRPf.calc_LRPObs(model,XobsS,biasBool,LRType,num_of_class,
                            yearsall,lrpRule1,normLRP,numLats,numLons,numDim)

### For testing data only (e-rule)
lrpobse = LRPf.calc_LRPObs(model,XobsS,biasBool,LRType,num_of_class,
                            yearsall,lrpRule2,normLRP,numLats,numLons,numDim)

### For testing data only (integrated gradients)
lrpobsig = LRPf.calc_LRPObs(model,XobsS,biasBool,LRType,num_of_class,
...
for osi in range(len(testingEnsembleMemberS_all)):
    ...
    lrp_os_z[osi,:,:,:] = LRPf.calc_LRPObs(model,testingEnsembleMemberSq,biasBool,LRType,
                                        num_of_class,yearsall,lrpRule1,
                                        normLRP,numLats,numLons,numDim)
    lrp_os_e[osi,:,:,:] = LRPf.calc_LRPObs(model,testingEnsembleMemberSq,biasBool,LRType,
                                        num_of_class,yearsall,lrpRule2,
                                        normLRP,numLats,numLons,numDim)
    lrp_os_ig[osi,:,:,:] = LRPf.calc_LRPObs(model,testingEnsembleMemberSq,biasBool,LRType,
...
for osti in range(len(testingEnsembleMemberS_10ye_all)):
    ...
    lrp_os_10ye_z[osti,:,:,:] = LRPf.calc_LRPObs(model,testingEnsembleMemberSq_10ye,biasBool,LRType,
                                        num_of_class,yearsall,lrpRule1,
                                        normLRP,numLats,numLons,numDim)
    lrp_os_10ye_e[osti,:,:,:] = LRPf.calc_LRPObs(model,testingEnsembleMemberSq_10ye,biasBool,LRType,
                                        num_of_class,yearsall,lrpRule2,
                                        normLRP,numLats,numLons,numDim)
    lrp_os_10ye_ig[osti,:,:,:] = LRPf.calc_LRPObs(model,testingEnsembleMemberSq_10ye,biasBool,LRType,
                                        num_of_class,yearsall,lrpRule3,
                                        normLRP,numLats,numLons,numDim)

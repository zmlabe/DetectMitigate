"""
Examine relationship between SWsfc and heatwaves
...
plt.yticks(np.arange(-50,51,1),map(str,np.round(np.arange(-50,51,1),2)),fontsize=10)
plt.xlim([0,80])
plt.ylim([-3,4])

plt.xlabel(r'\textbf{Count of Tn90 days in JJA}',fontsize=11,color='dimgrey')
plt.ylabel(r'\textbf{swup_sfc Anomaly [W/m2]}',fontsize=11,color='dimgrey')
...
plt.tight_layout()
plt.savefig(directoryfigure + 'Scatter_swup_sfc-Tn90_OS.png',dpi=300)

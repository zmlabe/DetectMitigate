"""
Calculate trend for OS for the Bowen Ratio but just for the CONUS
...
spear_mLH = 1000 * 2.45e6 * spear_mALLe * (1/86400) * (1/1000)
spear_hLH = 1000 * 2.45e6 * spear_hALLe * (1/86400) * (1/1000)
spear_osmLH = 1000 * 2.45e6 * spear_osmALLe * (1/86400) * (1/1000)
spear_osm_10yeLH = 1000 * 2.45e6 * spear_osm_10yeALLe * (1/86400) * (1/1000)

###############################################################################
###############################################################################
###############################################################################
### Get data for sensible heat
spear_mSH,lats,lons = read_primary_dataset(variq2,'SPEAR_MED',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_hSH,lats,lons = read_primary_dataset(variq2,'SPEAR_MED_ALLofHistorical',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_osmSH,lats,lons = read_primary_dataset(variq2,'SPEAR_MED_Scenario',monthlychoice,'SSP534OS',lat_bounds,lon_bounds)
spear_osm_10yeSH,lats,lons = read_primary_dataset(variq2,'SPEAR_MED_SSP534OS_10ye',monthlychoice,'SSP534OS_10ye',lat_bounds,lon_bounds)
lon2,lat2 = np.meshgrid(lons,lats)

###############################################################################
###############################################################################
###############################################################################
### Calculate Bowen Ratio (B=SH/LH)
spear_m = spear_mSH/spear_mLH
spear_h = spear_hSH/spear_hLH
spear_osm = spear_osmSH/spear_osmLH
spear_osm_10ye = spear_osm_10yeSH/spear_osm_10yeLH

###############################################################################
###############################################################################
###############################################################################
### Get data for global temperature
...
### Colorbar limits
barlim = np.arange(-1,1.1,0.25)
limit = np.arange(-1,1.01,0.01)
barlim2 = np.arange(-1,1.1,0.25)
limit2 = np.arange(-1,1.01,0.01)
label = r'\textbf{BOWEN RATIO CHANGE [unitless]}' 
...
plt.savefig(directoryfigure + 'GWL-%s_%s_%s_CONUS.png' % (selectGWLn,'BowenRatio',seasons[0]),dpi=300)

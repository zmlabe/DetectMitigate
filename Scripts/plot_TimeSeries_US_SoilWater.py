"""
Calculate timeseries of CONUS mean of column-integrated soil water
...
plt.fill_between(x=yearsh,y1=ave_GLOBE_minhh/1000,y2=ave_GLOBE_maxhh/1000,facecolor='k',zorder=1,
         alpha=0.4,edgecolor='none',clip_on=False) 
plt.plot(yearsh,ave_GLOBE_avghh/1000,linestyle='-',linewidth=2,color='k',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_Historical}')   

plt.fill_between(x=years,y1=ave_GLOBE_minh/1000,y2=ave_GLOBE_maxh/1000,facecolor='maroon',zorder=1,
         alpha=0.4,edgecolor='none',clip_on=False) 
plt.plot(years,ave_GLOBE_avgh/1000,linestyle='-',linewidth=2,color='maroon',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP585}')    

plt.fill_between(x=years,y1=ave_os_GLOBE_minh/1000,y2=ave_os_GLOBE_maxh/1000,facecolor='darkslategrey',zorder=1,
         alpha=0.4,edgecolor='none',clip_on=False) 
plt.plot(years,ave_os_GLOBE_avgh/1000,linestyle='-',linewidth=2,color='darkslategrey',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP534OS}')    

plt.fill_between(x=years,y1=ave_os_10ye_GLOBE_minh/1000,y2=ave_os_10ye_GLOBE_maxh/1000,facecolor='teal',zorder=1,
         alpha=0.4,edgecolor='none',clip_on=False) 
plt.plot(years,ave_os_10ye_GLOBE_avgh/1000,linestyle='-',linewidth=2,color='teal',
          clip_on=False,zorder=3,label=r'\textbf{SPEAR_MED_SSP534OS_10ye}')    

leg = plt.legend(shadow=False,fontsize=10,loc='upper center',
      bbox_to_anchor=(0.5,0.13),fancybox=True,ncol=2,frameon=False,
      handlelength=1,handletextpad=0.5)
for line,text in zip(leg.get_lines(), leg.get_texts()):
    text.set_color(line.get_color())

plt.xticks(np.arange(1920,2101,10),np.arange(1920,2101,10))
plt.yticks(np.round(np.arange(-100,101,0.1),2),np.round(np.arange(-100,101,0.1),2))
plt.xlim([1921,2100])
plt.ylim([-0.3,0.1])

plt.ylabel(r'\textbf{Column-integrated soil water [m; 1921-1950; %s]}' % (seasons[0]),
            fontsize=7,color='k')

plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_CONUS_SoilWater_historicalbaseline_%s_%s.png' % (seasons[0],reg_name),dpi=300)

...
for i in range(len(monthsall)):
    ...
    ### Save index
    directoryoutput = '/work/Zachary.Labe/Data/ClimateIndices/SND_BoxNA/'
    directoryoutput2 = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/SND_BoxNA_Obs/'
    np.savetxt(directoryoutput + 'SND_BoxNA_%s_%s_%s-%s_detrended.txt' % (variq,sliceperiod,
                                                                    yearmin,yearmax),np.c_[years,shi])
    np.savetxt(directoryoutput2 + 'SND_BoxNA_%s_%s_%s-%s_detrended.txt' % (variq,sliceperiod,
                                                                    yearmin,yearmax),np.c_[years,shi])
    print('\n========Calculated SND_BoxNA - %s =======\n' % monthsall[i])

"""
Calculate trend for OS with daily q (dry)
...
count10 = data.variables[varcount][:]
data.close()
...
count10_os = data_os.variables[varcount][:]
data_os.close()
...
count10_os10ye = data_os10ye.variables[varcount][:]
data_os10ye.close()

### Epochs for +- years around selected GWL
spear_am = count10
spear_aosm = count10_os
spear_aosm_10ye = count10_os10ye
...
limit = np.arange(0,21,1)
barlim2 = np.arange(-10,11,5)
limit2 = np.arange(-10,11,1)
if varcount == 'count10':
    label = r'\textbf{Count of qx10}' 
elif varcount == 'count05':
    label = r'\textbf{Count of qx05}' 

### Map world map
fig = plt.figure(figsize=(10,4))
ax = plt.subplot(231)
for qxt in fig.texts:
    qxt.set_visible(False)
...
plt.savefig(directoryfigure + 'GWL-%s_DryExtremes_%s_%s_%s-%s.png' % (selectGWLn,variq,seasons[0],varcount,variq),dpi=300)

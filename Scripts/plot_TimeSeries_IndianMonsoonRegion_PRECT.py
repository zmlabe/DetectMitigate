"""
Plots timeseries of precipitation over Indian Monsoon region
...
scenarioall = ['SSP585','SSP543OS','SSP543OS_10ye']
...
data,lat1,lon1 = read_primary_dataset(variq,'SPEAR_MED',monthlychoice,'SSP585',lat_bounds,lon_bounds)
...
###############################################################################
###############################################################################
### Calculate mean PRECT over Central Africa
latmin = 0
latmax = 40
lonmin = 60
lonmax = 100
...
NA_data1 = data[:,:,latq,:]
NA_data = NA_data1[:,:,:,lonq]
ave_NA_data = UT.calc_weightedAve(NA_data,latNA2)
...
maxd = np.nanmax(ave_NA_data,axis=0)
mind = np.nanmin(ave_NA_data,axis=0)
meand = np.nanmean(ave_NA_data,axis=0)
...
minens = [mind,min_os,min_os_10ye]
maxens = [maxd,max_os,max_os_10ye]
meanens = [meand,mean_os,mean_os_10ye]
colors = ['r','teal','maroon']
...
plt.yticks(np.round(np.arange(-48,48.1,0.5),2),np.round(np.arange(-48,48.1,0.5),2))
plt.xlim([2015,2100])
plt.ylim([4,8])

plt.ylabel(r'\textbf{Indian Monsoon Region PRECT [mm/day]}',
            fontsize=10,color='k')
plt.title(r'\textbf{Summer Asia Monsoon %s' % variq,fontsize=15,color='dimgrey')

plt.tight_layout()
plt.savefig(directoryfigure + 'IndianMonsoon_%s_EmissionScenarios_%s.png' % (variq,monthlychoice),dpi=300)

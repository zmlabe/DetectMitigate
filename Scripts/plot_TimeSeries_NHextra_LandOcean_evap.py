...
spear_m,lats,lons = read_primary_dataset('EVAP','SPEAR_MED',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_h,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_ALLofHistorical',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_osm,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_Scenario',monthlychoice,'SSP534OS',lat_bounds,lon_bounds)
spear_osm_10ye,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_SSP534OS_10ye',monthlychoice,'SSP534OS_10ye',lat_bounds,lon_bounds)
...
###############################################################################
### Get data - EVAP
spear_m_EVAP,lats,lons = read_primary_dataset('EVAP','SPEAR_MED',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_h_EVAP,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_ALLofHistorical',monthlychoice,'SSP585',lat_bounds,lon_bounds)
spear_osm_EVAP,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_Scenario',monthlychoice,'SSP534OS',lat_bounds,lon_bounds)
spear_osm_10ye_EVAP,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_SSP534OS_10ye',monthlychoice,'SSP534OS_10ye',lat_bounds,lon_bounds)

### Calculate anomalies
yearq = np.where((yearsh >= 1921) & (yearsh <= 1950))[0]
climoh_spear_EVAP = np.nanmean(np.nanmean(spear_h_EVAP[:,yearq,:,:],axis=1),axis=0)

spear_am_EVAP = spear_m_EVAP - climoh_spear_EVAP[np.newaxis,np.newaxis,:,:]
spear_aosm_EVAP = spear_osm_EVAP - climoh_spear_EVAP[np.newaxis,np.newaxis,:,:]
spear_aosm_10ye_EVAP = spear_osm_10ye_EVAP - climoh_spear_EVAP[np.newaxis,np.newaxis,:,:]
...
ocean_spear = UT.calc_weightedAve(spear_am_EVAP,lat2)
ocean_spear_os = UT.calc_weightedAve(spear_aosm_EVAP,lat2)
ocean_spear_os10ye = UT.calc_weightedAve(spear_aosm_10ye_EVAP,lat2)
...
plt.yticks(np.round(np.arange(-18,18.01,0.05),2),np.round(np.arange(-18,18.01,0.05),2))
plt.xlim([2030,2100])
plt.ylim([0,0.2])
plt.title(r'\textbf{NORTHERN HEMISPHERE [$<$60N]}',color='k',fontsize=15)

plt.ylabel(r'\textbf{Evaporation Anomaly [mm/day; 1921-1950]}',
            fontsize=10,color='k')

plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_EVAP_GlobeLandOcean_NHnoPole.png',dpi=300)
...
plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_EVAP_LandOcean_NHnoPole.png',dpi=300)
...
plt.title(r'\textbf{NORTHERN HEMISPHERE [$<$60N]}',color='k',fontsize=15)
plt.ylabel(r'\textbf{Evaporation Ratio [Land/Ocean]}',
            fontsize=10,color='k')

plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_ratio_LandOcean_NHnoPole_EVAP.png',dpi=300)

...
spear_m = np.empty((numOfEns,yearsf.shape[0],len(seasons),70,144))
spear_h = np.empty((numOfEns,yearsh.shape[0],len(seasons),70,144))
spear_osm = np.empty((numOfEns,yearsf.shape[0],len(seasons),70,144))
spear_osm_10ye = np.empty((numOfEns_10ye,yearsf.shape[0],len(seasons),70,144))
...
climoh_spear_jja = spear_h_mask[:,yearq,:,:,:]
climoh_spear = np.nanmax(climoh_spear_jja[:,:,:,:,:],axis=2) # max during each month for JJA
climoh_spear_mean = np.nanmax(climoh_spear[:,:,:,:],axis=1) # max of the TMAXabs over 1981-2010
...
for yrp in range(len(periods_15)):
    for i in range(lats.shape[0]):
        for j in range(lons.shape[0]):
            ...
            elif np.isnan(np.nanmax(spear_mask[:,yrq,:,i,j])):
                ssp585[yrp,i,j] = np.nan
                ssp585_os[yrp,i,j] = np.nan
                ssp585_os10ye[yrp,i,j] = np.nan
            else:
                ssp585[yrp,i,j] = 0.
                ssp585_os[yrp,i,j] = 0.
...
for i in range(len(periods_15)*3):
    ...
    m.drawlsmask(land_color=(0,0,0,0),ocean_color='mintcream',lakes=False,zorder=11)
...
plt.savefig(directoryfigure + 'PercentEnsMembers_TMAXabs_JJAabs.png',dpi=300)

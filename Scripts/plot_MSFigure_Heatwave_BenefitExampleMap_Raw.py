...
if any([variq == 'TMAX',variq == 'T2M']):
    ...
    barlim2 = np.arange(0,41,5)
    limit2 = np.arange(0,41,1)
    cmap = cm.jim_special_16_r.mpl_colormap
if variq == 'TMAX':
    if varcount == 'count90':
        label = r'\textbf{Years}' 
...
benefit_rawm[np.where(benefit_rawm > 0)] = np.nan
benefit_rawmask = (benefit_rawm*-1)

# cs = m.contourf(lon2,lat2,benefit_rawmask,limit2,
#                   extend='both',latlon=True)
cs = m.pcolormesh(lon2,lat2,benefit_rawmask,vmin=0,vmax=40,latlon=True)  
m.scatter(lonloc,latloc,marker='*',color='r',latlon=True,s=35,zorder=1000) 
cs.set_cmap(cmap)
...
benefit_thresholdm[np.where(benefit_thresholdm > 0)] = np.nan
benefit_thresholdmask = (benefit_thresholdm*-1)

# cs = m.contourf(lon2,lat2,benefit_thresholdmask,limit2,
#                 extend='both',latlon=True)    
cs = m.pcolormesh(lon2,lat2,benefit_thresholdmask,vmin=0,vmax=40,latlon=True)  
m.scatter(lonloc,latloc,marker='*',color='r',latlon=True,s=35,zorder=1000)                     
cs.set_cmap(cmap)
m.drawlsmask(land_color=(0,0,0,0),ocean_color='dimgrey',lakes=False,zorder=11)
m.drawcoastlines(color='darkgrey',linewidth=0.5,zorder=12)
...
benefit_filteringm[np.where(benefit_filteringm > 0)] = np.nan
benefit_filteringmask = (benefit_filteringm*-1)

### Make the plot continuous
# cs = m.contourf(lon2,lat2,benefit_filteringmask,limit2,
#                 extend='both',latlon=True)     
cs = m.pcolormesh(lon2,lat2,benefit_filteringmask,vmin=0,vmax=40,latlon=True)  
m.scatter(lonloc,latloc,marker='*',color='r',latlon=True,s=35,zorder=1000)                 
cs.set_cmap(cmap)
m.drawlsmask(land_color=(0,0,0,0),ocean_color='dimgrey',lakes=False,zorder=11)
m.drawcoastlines(color='darkgrey',linewidth=0.5,zorder=12)
...
plt.savefig(directoryfigure + 'MSFigure_Heatwave_BenefitTimeSeries_%s_ExampleMap_%s-%s_%s_Raw_none.png' % (variq,latloc,lonloc,monthlychoice),dpi=300)

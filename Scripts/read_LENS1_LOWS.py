"""
Function(s) reads in monthly data from LENS1_LOWS large ensemble for different 
variables using 40 ensemble members for all time periods
...
Usage
-----
    [1] read_LENS1_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)
"""

def read_LENS1_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper):
    """
    Function reads monthly data from LENS1_LOWS
    ...
    Usage
    -----
    read_LENS1_LOWS(directory,vari,sliceperiod,sliceshape,
                             slicenan,numOfEns,timeper)
    """
    print('\n>>>>>>>>>> STARTING read_LENS1_LOWS function!')
    ...
    for i,ensmember in enumerate(ens):
        filename1 = directory + 'LENS1_LOWS/monthly/%s/%s_%03d_1920-2100.nc' % (vari,vari,
                                                          ensmember)
        ...
        print('Completed: read *LENS1_LOWS* Ensemble Member --%s-- for %s' % (ensmember,vari))
    ...
    del membersvar
    print('Completed: read all LENS1_LOWS Members!\n')
    ...
    ###########################################################################
    ### Select years of analysis (1920-2100)
    if timeper == 'all':
        ...
        yearhistq = np.where((time >= 1920) & (time <= 2021))[0]
        print('HISTORICAL YEARS')
        print(time[yearhistq])
        histmodel = ensshape[:,yearhistq,:,:]
    ...
    print('>>>>>>>>>> ENDING read_LENS1_LOWS function!')    
...
# numOfEns = 40
# timeper = 'all'
# lat,lon,var = read_LENS1_LOWS(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)

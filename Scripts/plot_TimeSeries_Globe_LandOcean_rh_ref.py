"""
Calculate land ocean time series for RH over the globe
...
plt.yticks(np.round(np.arange(-5,2.1,0.5),2),np.round(np.arange(-5,2.1,0.5),2))
plt.xlim([1921,2100])
plt.ylim([-3,2])
plt.title(r'\textbf{Global Means}',color='k',fontsize=15)

plt.ylabel(r'\textbf{Relative Humidity Anomaly [Percent; 1921-1950]}',
            fontsize=10,color='k')

plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_rh_ref_GlobeLandOcean.png',dpi=300)
...
plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_rh_ref_LandOcean.png',dpi=300)
...
plt.xticks(np.arange(1920,2101,10),np.arange(1920,2101,10))
plt.yticks(np.round(np.arange(-100,26.01,2),2),np.round(np.arange(-100,26.01,2),2))
plt.xlim([2015,2100])
plt.ylim([-40,2])
...
plt.tight_layout()
plt.savefig(directoryfigure + 'TimeSeries_ratio_LandOcean_rh_ref.png',dpi=300)

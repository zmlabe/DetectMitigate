...
### Mask over the USA
spear_amL,maskobs = dSS.mask_CONUS(spear_mALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_ahL,maskobs = dSS.mask_CONUS(spear_hALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_aosmL,maskobs = dSS.mask_CONUS(spear_osmALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)
spear_aosm_10yeL,maskobs = dSS.mask_CONUS(spear_osm_10yeALL,np.full((spear_mALL.shape[1],spear_mALL.shape[2],spear_mALL.shape[3]),np.nan),'MEDS',lat_bounds,lon_bounds)

### Calculate latent heat (H = p*lv*E)
spear_m = 1000 * 2.45e6 * spear_amL * (1/86400) * (1/1000)
spear_h = 1000 * 2.45e6 * spear_ahL * (1/86400) * (1/1000)
spear_osm = 1000 * 2.45e6 * spear_aosmL * (1/86400) * (1/1000)
spear_osm_10ye = 1000 * 2.45e6 * spear_aosm_10yeL * (1/86400) * (1/1000)
...
plt.yticks(np.arange(-50,51,5),map(str,np.round(np.arange(-50,51,5),2)),fontsize=10)
plt.xlim([0,80])
plt.ylim([-15,20])

plt.xlabel(r'\textbf{Count of Tn90 days in JJA}',fontsize=11,color='dimgrey')
plt.ylabel(r'\textbf{LHFLX Anomaly [W/m2]}',fontsize=11,color='dimgrey')
...
plt.tight_layout()
plt.savefig(directoryfigure + 'Scatter_LHFLX-Tn90_OS.png',dpi=300)

...
### Colorbar limits
barlim = np.round(np.arange(-4,5,4),2)
limit = np.arange(-4,4.1,0.1)
label = r'\textbf{Trend in RH_ref [percent/decade] - 30 years after peak warming}' 
...
plt.savefig(directoryfigure + 'Trends_OS_RH_%s_CONUS.png' % (monthlychoice),dpi=300)
...
barlim = np.round(np.arange(-4,5,4),2)
limit = np.arange(-4,4.1,0.1)
label = r'\textbf{Trend in RH_ref [percent/decade] - 2071-2100}' 
...
plt.savefig(directoryfigure + 'Trends_OS_RH_%s_CONUS_last30yrs.png' % (monthlychoice),dpi=300)

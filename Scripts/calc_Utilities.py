...
-----
    ...
    [12] mk_test(x, alpha)
    [13] regions(name)
    [14] remove_ocean(data,lat_bounds,lon_bounds)
    [15] remove_land(data,lat_bounds,lon_bounds)
"""
...
def calc_weightedAve(var,lats):
    """
    Area weights any multidimensional array in numpy with at least [lat,lon]
    
    Parameters
    ----------
    var : 6d,5d,4d,3d,2d array of a gridded variable
    lats : 2d array of latitudes
    
    Returns
    -------
    meanvar : weighted average for 4d,3d,2d,1d array
    ...
    if var.ndim == 6:
        meanvar = np.empty((var.shape[0],var.shape[1],var.shape[2],var.shape[3]))
        for mo in range(var.shape[0]):
            for ens in range(var.shape[1]):
                for i in range(var.shape[2]):
                    for j in range(var.shape[3]):
                        ...
                        meanvar[mo,ens,i,j] = np.nansum(varmask*areamask) \
                                            /np.sum(areamask)  
...
def linearTrend(datavar,years,level,yearmn,yearmx):
    ...
    print('Shape of trend array is ---> %s!' % [datavar.shape])
...
def mk_test(x, alpha):
    ...
    print('\n>>> Completed: Finished mk_test function!')
    return trend, h, p, z
...
def remove_ocean(data,lat_bounds,lon_bounds):
    """
    Function to mask out ocean

    Parameters
    ----------
    data : only with latitude and longitude bounds of 96x144
    ...
    Usage
    -----
    datamask = remove_ocean(data,lat_bounds,lon_bounds)
    """
    print('\n>>> Using remove_ocean function! \n')
    ...
    print('*Completed: Finished remove_ocean function!')
    return datamask
...
def remove_land(data,lat_bounds,lon_bounds):
    """
    Function to mask out land
    ...
    Usage
    -----
    datamask = remove_land(data,lat_bounds,lon_bounds)
    """
    print('\n>>> Using remove_land function! \n')
    ...
    print('*Completed: Finished remove_land function!')
    return datamask
